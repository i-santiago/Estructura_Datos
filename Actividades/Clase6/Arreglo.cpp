...
void Arreglo<T>::Lectura()
{
    ...
    {
        cout << "\nIngrese la " << Indice + 1 << " calificacion: ";
        cin >> Datos[Indice];
    }
}
...
lleva a cabo, se mecrementa a Tam. */
template <class T>
...
template <class T>
int Arreglo<T>::Imprimir(T Valor){
    int Indice = 1, Resultado = -1;
    while((Indice < Tam) && (Datos[Indice] != Valor))
        Indice++;
        if (Indice < Tam){
        Resultado = Indice;
        cout <<InsertaDesordenado(T Valor);
        }
    return Resultado;
}
